...
/// Direct-entry resource — no delete route exists, only create and a `.../release` action.
class ReservedStockListScreen extends ConsumerWidget {
  const ReservedStockListScreen({super.key});

  Future<void> _release(BuildContext context, WidgetRef ref, String id) async {
    try {
      await ref.read(apiClientProvider).post('/$reservedStockResourcePath/$id/release');
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final recordsAsync = ref.watch(entityListProvider(reservedStockResourcePath));
    final productsAsync = ref.watch(entityListProvider('products'));
    final warehousesAsync = ref.watch(entityListProvider('warehouses'));

    return Scaffold(
      appBar: AppBar(title: const Text('Reserved Stock')),
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => context.push('/$reservedStockResourcePath/new'),
        icon: const Icon(Icons.add),
        label: const Text('New'),
      ),
      body: RefreshIndicator(
        onRefresh: () async => ref.invalidate(entityListProvider(reservedStockResourcePath)),
          ...
          data: (records) {
            ...
            return ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                final expiresAt = record['expiresAt']?.toString().split('T').first ?? '';
                return ListTile(
                  title: Text(product),
                  subtitle: Text(
                    '$warehouse · Qty: ${record['quantity']} · ${record['referenceType']} · Expires: $expiresAt · $status',
                  ),
